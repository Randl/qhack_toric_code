...
def get_toric_code(x, y, classical_bit_count=4, boundary_condition='matching'):
    assert boundary_condition in ('mixed', 'matching')
    if boundary_condition == 'matching':
        return ToricCodeMatching(x, y, classical_bit_count)
    elif boundary_condition == 'mixed':
        return ToricCodeMixed(x, y, classical_bit_count)
